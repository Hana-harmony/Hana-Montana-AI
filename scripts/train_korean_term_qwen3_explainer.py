...
DEFAULT_SOURCE_PATH = Path("data/training/korean_financial_term_explanation_sft.jsonl")
DEFAULT_DATA_DIR = Path("data/training/korean_financial_term_explanation_mlx")
DEFAULT_ADAPTER_DIR = Path("src/hannah_montana_ai/model_store/korean_term_qwen3_explainer_lora")
DEFAULT_REPORT_PATH = Path("reports/korean-term-qwen3-explainer-training.json")
    ...
    if not args.prepare_only:
        ...
        if executable is None:
            raise RuntimeError(
                ...
                "`uv run --extra llm-training mlx_lm.lora --help`로 의존성을 확인해야 함"
            )
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"Korean term Qwen3 LoRA training failed: {completed.returncode}")

    report = {
        "schema_version": "korean-term-qwen3-explainer-training/v1",
        "base_model": args.model,
        "training_dataset": str(args.source_path),
        "mlx_data_dir": str(args.data_dir),
        "adapter_dir": str(args.adapter_dir),
        "split_counts": split_counts,
        "training": training_result,
        "serving_note": (
            "운영에서는 Qwen3 GGUF를 띄우고 HANNAH_KOREAN_FINANCIAL_TERM_LLM_ENDPOINT로 연결한다."
        ),
    }
...
def prepare_mlx_chat_dataset(
    *,
    ...
    if len(rows) < 100:
        raise ValueError("Korean term Qwen3 학습에는 최소 100개 이상의 샘플이 필요함")
    data_dir.mkdir(parents=True, exist_ok=True)
    rows = sorted(rows, key=_stable_split_key)
...
def _stable_split_key(row: dict[str, object]) -> str:
    basis = json.dumps(
        {
            "term": row.get("term"),
            "sample_variant": row.get("sample_variant"),
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Korean term Qwen3-0.6B LoRA.")
    parser.add_argument("--source-path", type=Path, default=DEFAULT_SOURCE_PATH)
    parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR)
    parser.add_argument("--adapter-dir", type=Path, default=DEFAULT_ADAPTER_DIR)
    parser.add_argument("--report-path", type=Path, default=DEFAULT_REPORT_PATH)
    parser.add_argument("--model", default=DEFAULT_MODEL)
    parser.add_argument("--iters", type=int, default=360)
    ...
    parser.add_argument("--steps-per-eval", type=int, default=40)
