...
DEFAULT_SOURCE_PATH = Path("data/training/news_summary_wwi_sft.jsonl")
DEFAULT_DATA_DIR = Path("data/training/news_summary_wwi_mlx")
DEFAULT_ADAPTER_DIR = Path("src/hannah_montana_ai/model_store/news_summary_qwen3_lora")
DEFAULT_REPORT_PATH = Path("reports/news-summary-qwen3-training.json")
...
def main() -> None:
    ...
    if not args.prepare_only:
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"News summary Qwen3 LoRA training failed: {completed.returncode}")

    report = {
        "schema_version": "news-summary-qwen3-training/v1",
...
def prepare_mlx_chat_dataset(
    *,
    ...
    if len(rows) < 100:
        raise ValueError("News summary Qwen3 학습에는 최소 100개 이상의 샘플이 필요함")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train news summary Qwen3-0.6B LoRA.")
    parser.add_argument("--source-path", type=Path, default=DEFAULT_SOURCE_PATH)
    parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR)
    parser.add_argument("--adapter-dir", type=Path, default=DEFAULT_ADAPTER_DIR)
    parser.add_argument("--report-path", type=Path, default=DEFAULT_REPORT_PATH)
    parser.add_argument("--model", default=DEFAULT_MODEL)
    parser.add_argument("--iters", type=int, default=480)
