...
DEFAULT_OUTPUT = PROJECT_ROOT / "data/k_fnspid/v1"
...
def main() -> None:
    ...
    if not trading_dates:
        raise SystemExit("market daily price CSV is empty; export OmniLens prices first")
...
def load_prices(path: Path) -> list[DailyPrice]:
    ...
    if path.suffix == ".parquet":
        return [DailyPrice(**row) for row in pq.read_table(path).to_pylist()]
