from fastapi.testclient import TestClient

from hannah_montana_ai.main import app


def test_global_peer_match_api_returns_dynamic_grounded_popup_copy() -> None:
    client = TestClient(app)

    response = client.post(
        "/api/v1/market/global-peers/match",
        json={
            "stock_code": "196170",
            "stock_name": "알테오젠",
            "stock_name_en": "Alteogen",
            "market": "KOSDAQ",
            "peer_count": 3,
        },
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["success"] is True
    data = payload["data"]
    assert data["primary_peer"]["ticker"]
    assert data["stock_name_en"] == "Alteogen"
    assert "Alteogen" in data["headline"]
    assert data["source"] == "HANNAH_GLOBAL_PEER_HYBRID_RANKER"
    assert data["explanation_source"] == "GROUNDED_TEMPLATE_STRUCTURED_RAG"
    assert data["explanation_prompt_version"] == "global-peer-structured-rag-explainer-v8"
    assert len(data["comparisons"]) == 3
    assert len(data["key_strengths"]) == 4
    comparison_tickers = [comparison["peer"]["ticker"] for comparison in data["comparisons"]]
    assert data["primary_peer"]["ticker"] == comparison_tickers[0]
    assert len(set(comparison_tickers)) == 3
    assert len({comparison["dimension"] for comparison in data["comparisons"]}) == 3
    assert data["key_strengths"][0]["icon_key"] in {
        "memory",
        "foundry",
        "ai",
        "ecosystem",
        "semiconductor",
        "consumer_electronics",
        "software_platform",
        "financial_services",
        "payments",
        "biotechnology",
        "drug_delivery",
        "battery",
        "automotive",
        "telecommunications",
        "energy",
        "materials",
        "industrial",
        "commerce",
        "media",
        "global_business",
        "operational_scale",
    }
    assert "similarity score" not in data["summary"].lower()
    assert "confidence" not in data["summary"].lower()
