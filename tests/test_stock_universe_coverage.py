...
def test_stock_coverage_report_tracks_event_model_pseudo_training_coverage() -> None:
    ...
    assert pseudo_coverage["stock_candidate_event_training_sample_count"] == 739
    assert pseudo_coverage["stock_candidate_event_training_stock_count"] == 739
    assert pseudo_coverage["stock_candidate_per_stock_quota"] == 1
    assert pseudo_coverage["effective_event_training_stock_count_lower_bound"] == 739
    assert pseudo_coverage["stock_candidate_label_distribution"]["RISK"] == 275
    assert pseudo_coverage["stock_candidate_label_distribution"]["CONTRACT"] == 255
...
def test_stock_collection_shard_plan_targets_missing_candidate_coverage() -> None:
    ...
    assert report["planned_stock_count"] == 686
    assert report["shard_size"] == 100
    assert report["shard_count"] == 7
    assert report["planned_query_count"] == report["planned_stock_count"] * len(
        report["intents"]
    )
    assert report["priority_distribution"]["no_raw_no_candidate"] == 506
    assert report["priority_distribution"]["raw_without_candidate"] == 180
