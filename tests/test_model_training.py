...
def test_model_release_report_matches_source_reports() -> None:
    ...
    assert release_report["training"]["sample_count"] == 4690
    assert release_report["training"]["pseudo_labeled_sample_count"] == 1081
    ...
    assert release_report["pseudo_labeling"]["stock_candidate_accepted_count"] == 739
    ...
    assert stock_candidate_labeling["candidate_count"] == 13983
    assert stock_candidate_labeling["accepted_count"] == 739
    assert stock_candidate_labeling["accepted_stock_count"] == 739
    assert stock_candidate_labeling["accepted_count_by_primary_label"]["RISK"] == 275
    assert stock_candidate_labeling["accepted_count_by_primary_label"]["CONTRACT"] == 255
...
def test_stock_candidate_quota_experiment_report_tracks_release_expansion() -> None:
    ...
    assert profiles["current_release"]["overall_status"] == "pass"
    assert profiles["current_release"]["stock_candidate_labeling"]["accepted_count"] == 739
    assert profiles["current_release"]["stock_candidate_labeling"][
        "accepted_stock_count"
    ] == 739
    assert profiles["previous_release"]["stock_candidate_labeling"]["accepted_count"] == 500
    assert report["best_promotable_profile"]["accepted_stock_count"] == 739
...
def test_pseudo_label_monitoring_report_matches_source_reports() -> None:
    ...
    assert monitoring_report["candidate_funnel"]["raw_candidate_count"] == 61886
    assert monitoring_report["candidate_funnel"]["high_signal_candidate_count"] == 5099
    assert monitoring_report["candidate_funnel"]["promoted_count"] == 1081
    assert (
