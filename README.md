...
| --- | --- | --- | --- |
| 뉴스·공시 분석 | `financial_nlp_ml` + 파일 기반 `k_fnspid_impact_ml`, stock linker, 규칙 기반 What/Why/Impact | `POST /api/v1/alerts/analyze`, `/api/v1/intelligence/events` | [뉴스·공시 분석](docs/models/financial-alert-analysis.md), [K-FNSPID](docs/models/k-fnspid-market-impact.md) |
...
- 중요도는 의미 기반 모델과 K-FNSPID 시장반응 모델을 품질 gate 뒤에서 결합하며 모델 버전에 두 artifact를 기록한다.
