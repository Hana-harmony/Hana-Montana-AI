...
| --- | --- | --- | --- |
| 뉴스·공시 분석 | Hana Montana AI(KF-DeBERTa + K-FNSPID) + `financial_nlp_ml`, stock linker, Qwen 기반 What/Why/Impact | `POST /api/v1/alerts/analyze`, `/api/v1/intelligence/events` | [뉴스·공시 분석](docs/models/financial-alert-analysis.md), [K-FNSPID](docs/models/k-fnspid-market-impact.md), [도입 전후·연구 준비도](docs/models/k-fnspid-research-readiness.md) |
...
- 제목과 What/Why/Impact는 Qwen이 원문과 분류 신호로 생성한다. 필드 누락·근거 없는 숫자·한국어 잔존 등 품질 gate 실패 시 규칙 문구로 대체하지 않고 해당 분석을 실패 처리한다.
- 전문 번역은 OpenDART 구조를 보존하며 실패나 품질 저하는 `SOURCE_LANGUAGE_FALLBACK`과 품질 플래그로 명시한다. 발행 서비스는 `TRANSLATED` 전문만 공개한다.
