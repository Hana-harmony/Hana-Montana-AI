...
## 헬스체크
- `GET /health`는 API 프로세스 liveness를 확인한다.
- `GET /ready`는 `local_llm` 번역 모드에서 Qwen 서버 `/health`까지 확인하며, 의존성 장애 시 503을 반환한다.
- 로컬 compose healthcheck는 `/ready`를 사용해 번역 서버 없이 수집 작업이 시작되는 상태를 방지한다.
