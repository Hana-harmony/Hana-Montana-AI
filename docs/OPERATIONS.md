...
- 현재 artifact는 56,494건 수집 후보 중 weak-label 359건과 종목 후보 큐 701건을 이벤트 모델 학습에 반영했다.
- 종목 후보 큐 승격분은 per-stock quota 1건으로 제한해 701건이 701개 종목에 분산되도록 한다.
...
- 현재 56,494건 raw 후보 중 5,040건이 고신호 후보이고, teacher gate에서 3,737건이 탈락하며 weak-label 359건과 종목 후보 701건만 student 이벤트 모델 학습에 승격된다.
...
- 현재 event model pseudo training coverage는 701건, 701개 종목이며 supervised gold coverage로 간주하지 않는다.
...
- 현재 shard plan은 716개 `no_raw_no_candidate` 종목과 173개 `raw_without_candidate` 종목을 우선 수집 대상으로 둔다.
