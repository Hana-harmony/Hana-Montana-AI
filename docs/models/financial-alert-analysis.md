...
KF-DeBERTa 80% + 기존 모델 20% 확률 앙상블은 균형 공개 Test 933건에서 accuracy 0.8842, macro F1 0.8840이다. KF-DeBERTa 단독은 0.8850, 기존 Hana TF-IDF는 0.4423, `snunlp/KR-FinBERT-SC`는 0.7272이다. 운영 분포를 별도로 검증한 결과 실제 공시 Gold 600건은 accuracy 0.9233 / macro F1 0.8344, 실제 뉴스 Gold 80건은 accuracy 0.9000 / macro F1 0.8642다.
...
도입 전 동일 운영 평가와 비교하면 감성 정확도는 Benchmark `0.9688→0.9492`, 실제 뉴스 Gold `0.9750→0.9000`, Stock review Gold `0.9180→0.7880`으로 하락했다. 중요도는 v3에서 `거래정지·불성실공시 단독=HIGH`로 코드북을 바꿔 Benchmark 수치를 직접 증감으로 해석하지 않는다. 코드북이 유지된 실제 뉴스 Gold는 `0.9625→0.9500`, Stock review Gold는 `0.8480→0.8060`으로 하락했다. 대신 독립 공시 중요도 파이프라인은 910건에서 0.9989 / 0.9962를 기록한다. 공개 감성 Test와 공시 의미 중요도 개선, 기존 운영 분포 회귀가 동시에 존재하므로 모든 분포에서 우월하다고 주장하지 않는다.
