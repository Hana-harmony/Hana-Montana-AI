...
- Qwen3가 한국어 원문과 KF-DeBERTa 감성·K-FNSPID 시장영향·이벤트·중요도 신호를 받아 영문 제목과 What/Why/Impact를 생성한다. 신호는 관점 설정에만 사용하고 원문 사실처럼 인용하지 않는다. 영문 요약의 종결부호 누락은 의미를 바꾸지 않는 형식 정규화로 보완한다. 필수 필드 누락, 중복 문장, 한국어 잔존, 원문에 없는 숫자, 제목·요약 생략부호, 4단어 미만 조각 문장, 종목코드로 시작하는 문장은 이전 출력을 포함한 교정 요청으로 최대 1회 재생성 후 분석 실패로 처리하며 규칙 기반 요약으로 후퇴하지 않는다.
