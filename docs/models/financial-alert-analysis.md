...
- Qwen3가 한국어 원문과 KF-DeBERTa 감성·K-FNSPID 시장영향·이벤트·중요도 신호를 받아 영문 제목과 What/Why/Impact를 생성한다. 신호는 관점 설정에만 사용하고 원문 사실처럼 인용하지 않는다. 필수 필드 누락, 중복 문장, 한국어 잔존, 원문에 없는 숫자는 최대 1회 재생성 후 분석 실패로 처리하며 규칙 기반 요약으로 후퇴하지 않는다.
