...
- Hana-Omni-Connect-API가 수집한 KRX 외국인 보유 snapshot과 일별 시계열을 받아 금일 외국인 취득 수량이 한도에 도달할 가능성을 한도소진율 boundary로 반환한다.
...
- Hana-Omni-Connect-API가 전달한 `document_content_base64` 파일 payload 또는 외부 OCR/MTS upload gateway가 추출한 `extracted_text`를 받아 공통 envelope로 서류 검증 결과를 반환한다.
- `RESIDENCE_CERTIFICATE`, `APOSTILLE`, `REDUCED_TAX_APPLICATION`은 공용 `TaxDocumentPipeline`에서 템플릿 분류, Tesseract 전체·영역 OCR, parser/reviewer, 이미지 품질 검사를 같은 순서로 실행한다. 엔진 장애 시 임의 승인하지 않고 `OCR_ENGINE_UNAVAILABLE` 사유로 manual review 상태를 반환한다.
