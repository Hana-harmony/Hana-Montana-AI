...
현지 거래소 앱에서 한국 뉴스 전문의 고유 금융 용어를 클릭했을 때 외국인 투자자가 이해할 수 있는 짧은 영어 설명을 제공한다. 검증된 사전은 우선 사용하고, 미등록 한글 용어는 기사 근거를 붙여 Qwen3 설명기로 생성한다.
...
- API 프로세스 안에서 운영 LLM을 직접 적재하지 않는다. 운영은 Qwen3 GGUF HTTP 런타임만 호출한다.
...
- `source`: `DICTIONARY`, `LOCAL_OPEN_SOURCE_LLM_RAG`, `UNVERIFIED_CONTEXT`
...
- 실제 신조어 품질은 OmniLens 클릭 로그와 Qwen3 응답을 함께 보며 주기적으로 평가해야 한다.
