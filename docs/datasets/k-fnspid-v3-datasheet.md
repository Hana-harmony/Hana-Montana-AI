# K-FNSPID v3 Datasheet

## 요약

K-FNSPID v3는 한국 상장시장 뉴스·공시와 파일 기반 일별 시세를 연결한 연구·학습 데이터셋이다. 문서 발표 시각을 한국 거래 세션에 맞춰 유효 거래일로 정규화하고, 문서–종목 관계, 사건 cluster, 1·3·5거래일 시장반응, 시간 분할을 별도 Parquet으로 보존한다.

| 항목 | 값 |
| --- | ---: |
| 문서 | 550,662 |
| 뉴스 / OpenDART 공시 | 524,696 / 25,966 |
| 문서–종목 관계 | 819,772 |
| 시장영향 행 | 398,942 |
| 비혼입 대표 시장영향 행 | 130,566 |
| 일별 시세 | 10,691,998행, 2,800종목 |
| 문서 기간 | 2000-02-03~2026-07-13 |
| 시세 기간 | 2000-01-04~2026-07-13 |
| 평가 Gold | 뉴스 80 + 공시 600 = 680 |

정본 manifest는 `data/k_fnspid/v3/manifest.json`이며 모든 원천·Parquet 파일의 byte 크기와 SHA-256을 기록한다.

공시 Gold의 등급·우선순위·감성 override·제외 기준은 [K-FNSPID v3 공시 주석 코드북](k-fnspid-v3-annotation-codebook.md)에 고정한다.
...
시장영향 학습은 문서 전체가 아니라 비혼입 사건 대표행을 사용하며 Train 107,175, Validation 6,975, Test 10,750건이다.
...
- K-FNSPID v3 정본 annotation 수 680에는 포함하지 않는 별도 연구 평가 파일이다.
...
- 공시 전문 커버리지는 34.55%이며 공시 유형별로 균등하지 않다.
...
- 대용량 Parquet은 Git commit에 직접 넣지 않고 고정 버전 Release 자산과 manifest로 배포한다.
