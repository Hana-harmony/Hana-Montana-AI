...
## 현재 ML 검증 기준
- `reports/ml-training-report.json`은 3,609건 supervised 샘플, 1,081건 pseudo-label 샘플, 722건 supervised holdout 검증 결과를 기록한다.
- `reports/weak-distillation-report.json`은 61,886건 약지도 후보 중 5,099건을 고신호 후보로 선별하고, teacher gate를 통과한 weak-label 342건과 종목 후보 739건을 이벤트 모델 학습에 승격한 결과를 기록한다.
...
- 현재 benchmark 결과는 이벤트 recall 0.9922, 이벤트 macro F1 0.9977, 감성 accuracy 1.0, 중요도 accuracy 0.9375, 종목 accuracy 1.0이다.
...
- 현재 실제 뉴스 gold 결과는 이벤트 recall 0.9500, 이벤트 macro F1 0.9210, 감성 accuracy 0.9125, 중요도 accuracy 0.9250, 종목 accuracy 1.0이다.
- `reports/model-release-report.json`은 현재 모델 버전 `financial-ml-tfidf-logreg-20260610203855`의 전체 release gate와 pseudo-label consistency check를 `overall_status=pass`로 기록한다.
- `reports/pseudo-label-promotion-monitoring.json`은 고신호 후보 5,099건, teacher 탈락 3,868건, quota 보류 150건, 최종 승격 1,081건을 `overall_status=pass`로 기록한다.
- `reports/stock-coverage-report.json`은 universe 3,967개, raw 매칭 3,458개 종목, supervised 38개 종목, evaluation 57개 종목을 기록한다.
- `reports/stock-coverage-report.json`은 event-model-only pseudo 학습 coverage 739건, 739개 종목도 별도 섹션으로 기록한다.
- `reports/stock-collection-shard-plan.json`은 후보 큐와 gold가 없는 686개 종목, 7개 shard, 3,430개 Naver 쿼리를 기록한다.
- stock collection shard plan은 506개 `no_raw_no_candidate` 종목을 raw가 이미 있는 종목보다 먼저 수집 대상으로 둔다.
- `reports/stock-candidate-quota-experiment.json`은 calibrated current release 739건/739종목이 gate를 통과했고, risk/contract 확장 profile은 881건/685종목까지 확장됐지만 실제 뉴스 gold macro F1 gate를 통과하지 못했음을 기록한다.
- `reports/stock-training-candidate-report.json`은 검수 대기 후보 13,983건, 3,278개 종목을 기록하며 coverage gate를 `pass`로 기록한다.
