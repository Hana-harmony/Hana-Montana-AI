...
이 디렉터리는 `K-FNSPID v4`의 ACL Rolling Review 장문 익명 심사용 원고, 저자 공개 영문본과 한글 참고본을 함께 관리한다. 공개 시스템명은 `Hana Montana AI(KF-DeBERTa + K-FNSPID)`로 고정한다.
...
- `k-fnspid-v4-ko.tex`: 같은 실험·한계·윤리 내용을 반영한 한글 논문
...
- `output/pdf/k-fnspid-v4-arr-review.pdf`: 렌더링·검수된 제출 PDF
- `output/pdf/k-fnspid-v4-author-preprint.pdf`: 저자 공개 영문 PDF
- `output/pdf/k-fnspid-v4-ko.pdf`: 한글 PDF
...
- `k-fnspid-v4-ko.pdf`는 국내 검토와 내용 확인을 위한 저자 공개 참고본이며 ACL 심사 언어를 대체하지 않는다.
...
현재 상태는 형식·실험·재현성 기준의 학회 제출 패키지다. 감성 공개 Test는 과거 반복 조회되어 동일셋 재현 비교로만 보고하며 독립 SOTA를 주장하지 않는다. 독립 금융전문가 다중 주석도 없으므로 외부 Gold 타당성이나 공시 가격반응 SOTA를 주장하지 않는다.
