...
한국 금융 NLP는 공개 뉴스 감성 데이터는 존재하지만, 장기간 뉴스·공시를 국내 전 종목 시세와 연결하고 발표 세션·사건 중복·미래 누수를 함께 통제한 공개 연구 자원이 부족하다. 본 연구는 Naver 뉴스 524,696건, OpenDART 공시 25,966건, 한국 상장종목 일별 시세 10,691,998행을 연결한 K-FNSPID v3를 제시한다. 데이터셋은 550,662개 문서, 819,772개 문서–종목 관계, 398,942개 시장영향 행과 130,566개 비혼입 사건 대표행으로 구성된다. 공시 전문 부족을 완화하기 위해 OpenDART 원문을 8,972건까지 확장하고, 모델 학습 입력과 URL이 겹치지 않는 기본·스트레스 공시 Gold 910건을 구축했다. 발표시각을 한국 거래 세션의 유효 거래일로 정규화하고 7일 embargo를 둔 시간순 Train/Validation/Test를 사용한다. 감성은 KF-DeBERTa LoRA와 기준선 확률 앙상블, 의미 중요도는 전문 약지도 원천에서 Validation으로 선택한 제목+요약 모델과 존속위험 정책, 가격반응 등급은 class-balanced focal loss와 ordinal CDF loss를 결합한 KF-DeBERTa LoRA 및 Validation 전용 log-prior 보정으로 모델링한다. 모든 비교는 동일 Test 문서 집합에서 수행하고, paired bootstrap, exact McNemar, calibration error를 함께 보고한다. 가격반응 라벨은 의미 중요도와 분리하며 인과적 중요도나 수익 신호가 아니라 관측 시장 충격의 약한 순서형 대리변수로 제한해 해석한다.
...
- FININ과 2026년 뉴스 융합 주가예측 연구는 뉴스 상호작용·과거 시세·종목 관계를 함께 쓰는 다중모달 예측이 텍스트 단독 분류보다 강한 최신 방향임을 보여준다. K-FNSPID v3의 현재 가격반응 실험은 텍스트 단독 보조 신호이므로 이들 수익률·MAE 결과와 수치 순위를 주장하지 않는다.
...
| K-FNSPID v3 | 한국 뉴스·공시–주가 연결 | 문서 550,662, 시세 10,691,998 | 동일 Test에서 자체 기준선과 비교 |
...
세부 원천, schema, 편향, 삭제·버전 정책은 [K-FNSPID v3 Datasheet](../datasets/k-fnspid-v3-datasheet.md)에 고정한다.

### 3.2 공시 원문 확장

기존 전문 학습셋의 공시 원문은 273건뿐이었다. OpenDART 원문 API를 완전 pagination 목록과 연결하고, 32MB 응답 상한, timeout, 재시도, ZIP 검증, DART 상태 분류를 적용해 전문 학습셋의 공시 원문을 8,976건까지 확장했다. 이 중 K-FNSPID raw 문서 URL과 연결되는 OpenDART 원문은 8,972건이다. 최종 추가 회수는 후보 5,065건을 시도해 4,976건을 저장했으며, 응답 실패 89건의 사유를 보고서에 보존했다.

수집 원문 라벨은 Gold가 아니다. `RULE_WEAK_SUPERVISION_V2`와 `UNREVIEWED_WEAK_LABEL`로 표시하고, Gold URL을 제거한 실제 공시 8,302건은 의미 중요도 후보의 supervised 학습에만 사용한다. 감성 학습과 평가 정답에는 사용하지 않는다. 기존의 모든 공시 HIGH 강제 규칙은 정기/행정 LOW, 지배구조 MEDIUM, 계약·자본·실적 HIGH, 치명 위험 CRITICAL 코드북으로 교체했다.
...
공시 Gold는 [공개 코드북](../datasets/k-fnspid-v3-annotation-codebook.md)으로 판정 가능한 사례만 Codex가 검수했다. 등급 우선순위, 감성 override, 근거 필드, 모호 사례 제외, 종목당 상한을 고정했다. 단일 Codex 검수이므로 사람 전문가 간 일치도를 주장하지 않는다.
...
구간은 LOW `<0.20`, MEDIUM `<0.45`, HIGH `<0.75`, CRITICAL `>=0.75`다. 미래 값은 정답 생성에만 사용한다.
...
### 6.4 가격반응 등급 Transformer

- KF-DeBERTa base + LoRA r=16
- 최대 256토큰
- class-balanced focal cross entropy, gamma 1.5
- ordinal CDF MSE 가중치 0.30
- label smoothing 0.02
- device batch 8, gradient accumulation 4, 유효 batch 32
- 초기 learning rate 2e-4, cosine scheduler, optimizer step의 warmup 8%
- weight decay 0.01, gradient norm 1.0, Validation macro F1 early stopping patience 1 epoch
- class-balanced loss에 따른 prior shift를 줄이기 위해 log class-prior 강도 0.00~2.00을 Validation macro F1로만 선택하고 raw·보정 성능을 함께 보고
- seed 17/42/73 반복 실험, 보정된 Validation macro F1만으로 배포 seed 선택
...
### 8.3 K-FNSPID v3 시간 Test

<!-- MULTISEED_RESULT_TABLE -->

| 모델/run | 선택 근거 | accuracy | macro F1 | QWK |
| --- | --- | ---: | ---: | ---: |
| TF-IDF Logistic | Train-only 기준선 | 0.4643 | 0.3429 | 0.3141 |
| KF-DeBERTa LoRA seed 17 | Validation 0.3703 | 0.5190 | 0.3724 | 0.4704 |
| KF-DeBERTa LoRA seed 42 | Validation 0.3704 | 0.5030 | 0.3927 | 0.4627 |
| KF-DeBERTa LoRA seed 73 | Validation 0.3788, 선택 | 0.5095 | 0.3820 | 0.4694 |
| KF-DeBERTa LoRA 3-seed 평균±표본표준편차 | 보고용 | 0.5105±0.0080 | 0.3824±0.0102 | 0.4675±0.0042 |

선택 seed 73과 기준선의 거래일 cluster bootstrap 2,000회 차이 95% CI는 accuracy [0.0351, 0.0557], macro F1 [0.0256, 0.0536], QWK [0.1331, 0.1776]으로 모두 0을 넘었고 exact McNemar p=1.70e-20이었다. Brier score는 0.6450에서 0.6129로 낮아졌으나 ECE는 0.0491에서 0.0662로 높아졌다. NEWS 10,160건의 macro F1은 0.3436→0.3847로 개선된 반면 DISCLOSURE 590건은 0.3006→0.2211로 하락했다. 전체 우월성은 뉴스 비중이 큰 Test 분포에서의 결과이며 공시 가격반응 우월성으로 확대하지 않는다.

기준선은 Train 107,175건으로만 적합한 공정 비교에서 Test accuracy 0.4643, macro F1 0.3429, quadratic kappa 0.3141다. Transformer 반복 seed·통계 결과는 `reports/k-fnspid-transformer-multiseed-report.json`과 `reports/k-fnspid-research-evaluation.json`의 고정 값으로 이 표에 반영한다.

## 9. Ablation과 도입 전후

### 9.1 TF-IDF 입력·학습량 절제
...
선형 TF-IDF에서는 전문 추가가 제목+snippet보다 낮았다. 수집량 확장이 시장영향 예측을 자동 개선한다는 주장을 배제하며, 전문은 공시 의미 중요도와 Transformer 문맥 표현에 사용한다. 학습량 증가는 2만 대비 macro F1을 높였지만 5만 이후 단조 증가하지 않아 분포 이동과 약한 라벨 잡음이 병목임을 시사한다.
...
| 공시 원문 273→8,972 | 공시 문맥·종목 연결 개선 |
| source prefix | 뉴스와 공시의 분포 차이 명시 |
| Train 20,000→107,175 | 축소 학습 편향 제거 |
| max length 128→256 | 긴 공시·뉴스 문맥 보존 |
...
따라서 현재 산출물은 재현 가능한 데이터셋·시스템 논문 초안과 내부 기술 보고서에는 충분하지만, 독립 금융 전문가 2인 이상 주석과 외부 DOI 저장소 동결 전에는 완전한 동료심사 제출본으로 표시하지 않는다.

## 12. 산출물

- dataset manifest: `data/k_fnspid/v3/manifest.json`
- datasheet: `docs/datasets/k-fnspid-v3-datasheet.md`
- baseline report/predictions: `reports/k-fnspid-impact-training-report.json`, `reports/k-fnspid-impact-test-predictions.jsonl`
- Transformer run reports/predictions: `reports/k-fnspid-transformer-*`
