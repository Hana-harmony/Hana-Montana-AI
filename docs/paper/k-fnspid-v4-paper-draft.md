...
기존 전문 학습셋의 공시 원문은 273건뿐이었다. OpenDART 원문 API를 완전 pagination 목록과 연결하고, 32MB 응답 상한, timeout, 재시도, ZIP 검증, DART 상태 분류를 적용해 전문 학습셋의 공시 원문을 8,976건까지 확장했다. 이 중 K-FNSPID raw 문서 URL과 연결되는 OpenDART 원문은 8,972건이다. 최종 추가 회수는 5,082건을 시도해 4,976건을 저장했으며, 실패 106건을 본문 부족 51건, DART 상태 013 1건, 상태 014 37건, 공개 뷰어 metadata 부재 17건으로 보존했다.
