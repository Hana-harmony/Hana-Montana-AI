...
| KR-FinBERT-SC | 한국 금융 감성 | 중복 제거 본 연구 Test 932 재평가 | 동일 문서·라벨에서 직접 비교 |
...
- 공개 분할: 정규화 중복·충돌 제거 후 Train 7,407 / Validation 932 / Test 932
- 선택: Validation을 Calibration 467 / Selection 465로 나누고 Selection에서 단독 LoRA를 잠금
- 승격: 후보 잠금에 Test·Gold 미사용, 공개 Test macro F1 0.85 이상, KR-FinBERT-SC 이상, 뉴스·공시 운영 Gold accuracy 0.90·macro F1 0.80 이상

Calibration으로 학습한 Logistic stacker와 source logit bias, 80:20 앙상블도 Validation Selection에서 비교하지만 잠근 단독 LoRA보다 낮거나 운영 Gold gate를 통과하지 못하면 서빙하지 않는다.
...
| --- | ---: | ---: | ---: |
| Hana TF-IDF Logistic | 932 | 0.4775 | 0.4415 |
| KR-FinBERT-SC | 932 | 0.7361 | 0.7266 |
| 잠근 KF-DeBERTa LoRA | 932 | 0.8852 | 0.8849 |
| KF-DeBERTa 80% + TF-IDF 20% | 932 | 0.8841 | 0.8838 |
| Source-bias 보정 | 932 | 0.8723 | 0.8724 |
| Logistic stacker | 932 | 0.8391 | 0.8331 |

현 프로토콜은 Test와 운영 Gold를 후보 선택에 사용하지 않고 Validation Selection에서 단독 LoRA를 잠갔다. 이 후보는 뉴스 Gold 80건에서 accuracy 0.8625 / macro F1 0.8308, 공시 Gold 600건에서 0.9150 / 0.8084를 기록했다. 뉴스 accuracy 0.90 gate에 미달하므로 신규 후보를 승격하지 않는다. 후보와 KR-FinBERT-SC의 2,000회 paired bootstrap macro F1 차이 평균은 0.1580이고 고정 예측 구간은 `[0.1265, 0.1899]`, exact McNemar의 계산값은 `p=9.81e-19`이다. 기존 TF-IDF 대비 차이는 0.4438, 구간은 `[0.4055, 0.4816]`, 계산값은 `p=1.34e-73`이다. 과거 benchmark가 Test 최고점으로 후보를 선택했고 개발 중 반복 조회했으므로 이 구간과 p값의 명목 오류율은 보장되지 않는다. 따라서 기술적 동일셋 재현 차이만 보고하며 확증적 통계 우위, 독립 확증 또는 한국 금융 감성 전역 SOTA를 주장하지 않는다.
...
| 감성 Validation Calibration/Selection 분리 | Test·운영 Gold를 결합기 선택에서 배제 |
