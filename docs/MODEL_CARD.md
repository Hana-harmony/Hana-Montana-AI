...
## 모델명
`financial-ml-tfidf-logreg-20260605045653`
...
- raw 후보에서 보수적 종목명·종목코드 매칭으로 확인한 종목 수: 3,101개
- 학습 승격 후보 큐 샘플 수: 11,011건
- 학습 승격 후보 큐 종목 수: 2,912개
...
- 종목 후보 큐에서는 teacher gate와 종목별 quota를 통과한 `RISK` 236건, `CONTRACT` 220건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 45건, `EARNINGS` 23건, `MACRO` 14건을 이벤트 모델 학습에 추가했다.
- 종목 후보의 per-stock quota는 1건으로 제한해 658건이 658개 종목에 분산되도록 했다.
- `stock-candidate-quota-experiment`에서 이전 release profile은 500건/500종목으로 통과했고, risk/contract per-stock 2 profile은 820건/624종목까지 늘었지만 실제 뉴스 gold recall gate를 통과하지 못했다. current release는 658건/658종목으로 실제 뉴스 gold gate를 통과해 best profile로 선택했다.
...
- 51,505건 raw 후보 중 4,985건이 고신호 후보로 남았다.
- teacher confidence 또는 weak-label 합의 기준에서 3,619건이 탈락했다.
- weak-label distillation에서는 `RISK` 140건, `CONTRACT` 180건, `CORPORATE_ACTION` 40건을 student 이벤트 모델 학습에 승격했다.
- 종목 후보 큐에서는 `RISK` 236건, `CONTRACT` 220건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 45건, `EARNINGS` 23건, `MACRO` 14건을 teacher gate로 추가 승격했다.
...
- raw 후보는 3,101개 종목까지 매칭되므로 다음 단계는 raw 후보를 종목별·라벨별로 검수해 supervised/gold 데이터로 승격하는 것이다.
- 후보 큐는 2,912개 종목을 포함하지만 약지도 기반 검수 대기 데이터이므로 gold label로 직접 사용하지 않는다.
- coverage review plan은 2,000개 종목의 검수 과제를 만들지만 아직 검수자 승인이 없으므로 supervised coverage gate를 통과시키지 않는다.
- 현재 artifact는 후보 큐 중 658개 종목의 658건만 teacher gate를 통과한 event-model-only pseudo-label로 제한 투입했다.
