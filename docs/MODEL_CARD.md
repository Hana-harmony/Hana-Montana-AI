...
- 설명 생성: 피어 선택은 hybrid ranker가 담당하고, headline/summary는 구조화된 peer fact를 근거로 생성하는 Structured RAG 설명기로 분리했다. 기본 serving은 deterministic grounded template이며, `HANNAH_GLOBAL_PEER_EXPLANATION_MODE=local_llm`과 OpenAI-compatible local endpoint가 켜진 경우에만 Qwen3 설명 LLM을 사용한다. 일반 피어 문구는 `Maps Closest To`와 `closest US-listed reference peer`로 보수화하고, 검수 anchor는 별도 문구를 유지한다. LLM 출력은 display name, peer명, ticker, 섹터/산업/사업모델, 금지 투자문구, headline exact match, summary exact match, 반복 단어 검증을 통과해야 하며 실패하면 template으로 fallback한다.
...
- raw LLM 생성 검증: `scripts/evaluate_global_peer_qwen3_generation.py`로 대표 30종목을 실제 `mlx_lm.generate` 호출로 평가했다. 품질 하드닝 후 v7 strict gate 기준 JSON valid 30/30, exact headline 30/30, exact summary 30/30, grounded 30/30, pass rate 1.0을 기록했다. serving은 exact headline/summary gate를 통과하지 못하면 deterministic template으로 fallback한다. 이전 raw 직접 생성은 대표 10개 모두 grounded fail이었고, v4/v5/v6 개선 과정에서 이름 반복·미제공 숫자 생성·사용자 노출 점수 문구·한글 종목명 문제를 발견해 summary exact gate, 영어 표시명, 점수 비노출 guard를 추가했다.
...
- 한국 금융 용어 해설 엔진은 `k-finance-term-rag-v2`이며, seed 사전, 기사 문맥 RAG, OpenAI web search fallback, confidence gate를 분리한다. 현재 `reports/korean-financial-term-explanation-eval.json` 기준 샘플 11건 정확도 1.0, 사전 커버리지 0.909091, 캐시 가능률 0.909091다. `초전도체주`, `로봇주`, `2차전지주`, `저PBR` 같은 고빈도 테마/정책 표현은 dictionary-backed explanation으로 승격했다.
