...
- 협력사용 `OMNI_CONNECT_API_KEY`는 요구하거나 저장하지 않는다.
...
- 외부 운영 credential은 GitHub Secrets 또는 배포 환경 Secret Manager에서 주입한다. OmniConnect 연계 유지보수 토큰은 동일 OCI 호스트의 영속 루트키에서 자동 파생한다.
...
- 모델 변경은 기능별 모델 상세 문서와 구현 기록에 남긴다.
